...
Provision server-custody signing keys (KMS-wrapped, never returned to the agent), derive addresses, and **sign + broadcast native transactions** for Bitcoin, Solana, XRP, Cardano, and Tron using the 1Claw SDK. Private keys are stored in the org's `__agent-keys` vault and never leave the HSM/TEE boundary — the agent only submits an intent.
...
All private keys are generated by the vault and stored envelope-encrypted — a per-key DEK wrapped by the org's KEK, which lives in Google Cloud KMS (HSM on paid tiers) — in the org's `__agent-keys` vault at `agents/{id}/chains/{chain}/private_key`. They are never returned to callers; only the public key and derived address are. The vault (or the Shroud TEE for agents with `intents_require_tee`) unwraps the key to sign and drops it. These are `custody: server` keys — 1Claw can sign on the agent's behalf after policy; see [Custody](https://docs.1claw.co/docs/security/custody) for the threshold and passkey-owner models.

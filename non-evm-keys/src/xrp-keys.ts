/**
 * 1Claw SDK — XRP Signing Key
 *
 * Provisions an HSM-backed Ed25519 key and derives an XRP classic address.
...
async function main() {
    ...
    if (res.error) {
        ...
        if (msg.includes("already") || msg.includes("409")) {
            ...
            if (!existing) {
                console.error("No active xrp key found");
                process.exit(1);
            }
    ...
    console.log("\n--- XRP Address Info ---\n");
    console.log(
        "  Format:   XRP classic address (starts with r, base58check with XRP alphabet)",
    );
    console.log(`  Explorer: https://xrpscan.com/account/${address!}`);
    console.log("  Faucet:   https://faucet.devnet.rippletest.net");

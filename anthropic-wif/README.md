...
3. **Decode** the JWT and assert that the `kid` resolves against `https://api.1claw.xyz/.well-known/jwks.json`.
...
[3] Verified kid 'rs256-v1' is published at https://api.1claw.xyz/.well-known/jwks.json
...
- Tokens carry `iss=https://api.1claw.xyz`, `sub=agent:<uuid>`, the requested `aud`, and a `kid` that resolves in the published JWKS.

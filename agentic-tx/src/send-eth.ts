...
async function main() {
    ...
    console.log("\n⛓️  Signing ETH transfer on Ethereum mainnet (chain_id=1)\n");
    ...
    if (result.error) {
        ...
        if (msg?.includes?.("insufficient") || msg?.includes?.("funds")) {
            console.error("❌ Insufficient funds. Fund your agent address first.");
    ...
    console.log(`\n  Explorer:  https://etherscan.io/tx/${tx.tx_hash}\n`);
}
